...
void loadData(string filename) {
    ...
    mem.reachable4 = new int[280000 * 4];
    memset(mem.visit4, false, 4 * 280000 * sizeof(bool));
    memset(mem.reachable4, -1, 4 * 280000 * sizeof(int));//初始化的长度是否正确？
    mem.tcount = 0;
...
void dense4(int i) { 
    ...
    int *reachable = mem.reachable4 + idx * 280000;
...
void dense1() {
    ...
    int *path = mem.path4;
    int count = 0;
    for(int i = 0; i < 280000 && count < 280000; i++) if(graph.strlen[i] != 0){
        count++;
        path[0] = i;
        prune_fg(i, i, visit, reachable, 1);
        prune_bg(i, i, visit, reachable, 1);
