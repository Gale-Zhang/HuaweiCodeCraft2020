...
void check4(int i) { 
    ...
    bool *visit;
    int *reachable;
    visit = mem.visit4 + idx * graph.pnum;
    reachable = mem.reachable4 + idx * graph.pnum;
    mem.path[idx].push_back(i);
    prune_fg(i, i, visit, reachable, 1);
    prune_bg(i, i, visit, reachable, 1);
    for(int j = graph.bgp[i]; j < graph.bgp[i + 1]; j++) {
        reachable[graph.bg[j]] = -2;
    }
    dfs(i, i, visit, reachable, mem.path[idx]);
    mem.path[idx].pop_back();
    for(int j = graph.bgp[i]; j < graph.bgp[i + 1]; j++) {
