...
void dfs(int cur, int target, bool *visit, int *reachable) {
	int thid = tid[pthread_self()];
	for(int i = gStart[cur]; i < gStart[cur + 1]; i++) {
		int t = graph[i];
		if(id[t] < id[target]) continue;
		if(reachable[t] == -2 && !visit[t]) {
			path[thid].push_back(t);
			if(path[thid].size() > 2) {
				vector<int> temp;
				for(auto p : path[thid]) {
					temp.push_back(id[p]);
				}
				bucket[temp.size() - 3][thid].push_back(temp);
			}
			path[thid].pop_back();
		}
		if(visit[t] || (reachable[t] != target && reachable[t] != -2)) continue;
		if(path[thid].size() == 6 || t == target) continue;
		visit[t] = true;
		path[thid].push_back(t);
...
void* check(void* args) {
	int a[] = {0,6250,13000,19000};
	int b[] = {6250,13000,19000,25000};
	...
	int st = a[t];//t * idLen / T_NUM;
	int end = b[t];//t == (T_NUM - 1 ) ? idLen : (t + 1) * idLen / T_NUM;
	int time = clock();

	cout<<"thread "<<t<<" ["<<st<<", "<<end<<")"<<endl;
	...
	cout<<"thread "<<t<<" elapsed time: "<<(clock() - time) / 1000<<endl;
}
...
void checkCircle() {
	...
	for(int i = 0; i < T_NUM; i++) {
		if(pthread_join(tids[i], NULL) != 0) {
			cout<<"finish thread "<<i<<" error!"<<endl;
		}
	}
}
