...
void *check(void *arg) {
    ...
    bool *visit = mem.visit + idx * VNUM;//(bool*)malloc(VNUM);
    char *distance = mem.distance + idx * VNUM;//(char*)malloc(VNUM);
    // memset(visit, false, VNUM * sizeof(bool));
    // memset(distance, 10, VNUM * sizeof(char));
    int *update = mem.update + idx * 125000;//(int*)malloc(125000);
    int *path = mem.path + idx * 7;//(int*)malloc(7);

    //Init answer buffer
    ab.ans[idx][0] = mem.ans3 + idx * 5 * MB;
    ab.ans[idx][1] = mem.ans4 + idx * 10 * MB;
    ab.ans[idx][2] = mem.ans5 + idx * 20 * MB;
    ab.ans[idx][3] = mem.ans6 + idx * 35 * MB;
    ab.ans[idx][4] = mem.ans7 + idx * 70 * MB;
...
void checkCircle() {
    ...
    mem.visit = (bool*)malloc(VNUM * TNUM);
    mem.distance = (char*)malloc(VNUM * TNUM);
    memset(mem.visit, false, TNUM * VNUM);
    memset(mem.distance, 10, TNUM * VNUM);
    mem.update = new int[125000 * TNUM];
    mem.path = new int[TNUM * 7]; /////>??????????????????????????????new 就没错？？？？

    //Init answer buffer
    mem.ans3 = (char*)malloc(5 * MB * TNUM);
    mem.ans4 = (char*)malloc(10 * MB * TNUM);
    mem.ans5 = (char*)malloc(20 * MB * TNUM);
    mem.ans6 = (char*)malloc(35 * MB * TNUM);
    mem.ans7 = (char*)malloc(70 * MB * TNUM);
