...
void loadData(string filename) {
    ...
    memset(graph.strlen, 0, sizeof(int) * 280000);
...
// void prune_fg(int cur, int target, bool * visit, int *distance, int dis) {
...
//         if(dis == 3) continue;
//         visit[t] = true;
//         prune_fg(t, target, visit, distance, dis + 1);
//         visit[t] = false;
//     }
// }

void prune_bg(int cur, int target, bool *visit, int *distance, int dis) {
    for(int i = 0; i < graph.bgl[cur]; i++) {
        ...
        if(dis == 3) continue;
        visit[t] = true;
        prune_bg(t, target, visit, distance, dis + 1);
        visit[t] = false;
    }
}

void dfs_dense(int cur, int target, bool *visit, int *distance, int *path, int len, int idx) {
    for(int i = 0; i < graph.fgl[cur]; i++) {
        int t = graph.fg[cur][i];
        if(t < target) break;
        ...
        visit[t] = true;
        path[len] = t;
        dfs_dense(t, target, visit, distance, path, len + 1, idx);
        visit[t] = false;
    }
}
...
void dense4(int i) { 
    ...
    path[0] = i;
    //prune_fg(i, i, visit, distance, 1);
    prune_bg(i, i, visit, distance, 1);
    for(int j = 0; j < graph.bgl[i]; j++) {
        distance[graph.bg[i][j]] = -2;
    }
    //500ms
    dfs_dense(i, i, visit, distance, path, 1, idx);
    for(int j = 0; j < graph.bgl[i]; j++) {
...
void dense1() {
    ...
    for(int i = 0; i < 280000 && count < graph.pnum; i++) if(graph.strlen[i] != 0){
        ...
        //500ms
        dfs_dense(i, i, visit, distance, path, 1, 0);
