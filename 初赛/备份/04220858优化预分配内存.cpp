...
void loadData(string filename) {
    ...
    memset(mem.reachable4, -1, 4 * graph.pnum);
    mem.tcount = 0;
...
int * temp = new int[10];
void dfs(int cur, int target, bool *visit, int *reachable, int *path, int len) {
    for(int i = graph.fgp[cur]; i < graph.fgp[cur + 1]; i++) {
        ...
        if(reachable[t] == -2 && !visit[t]) {
            ...
            if(len > 1) {
                ...
                memcpy(temp, path, (len + 1) * sizeof(int));
                graph.bucket[graph.rr[target] + graph.pnum * (len - 2)].push_back(temp);
            }
        }
        ...
        visit[t] = true;
        path[len] = graph.id[t];
        dfs(t, target, visit, reachable, path, len + 1);
        visit[t] = false;
    }
}
...
void check4(int i) { 
    ...
    dfs(i, i, visit, reachable, path, 1);
    for(int j = graph.bgp[i]; j < graph.bgp[i + 1]; j++) {
