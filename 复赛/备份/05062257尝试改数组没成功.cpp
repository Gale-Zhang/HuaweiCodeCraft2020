...
void loadData(string filename) {
    ...
    while(cur < bufLen) {
        ...
        if(idxmap.find(st) == idxmap.end()) {
            ...
            str[stIdx] = sb + offset;
            memcpy(sb + offset, buffer + last, cur - last);
    ...
    for(int i = 0; i < graph.pnum; i++) {
        ...
        graph.in[i] = (Line*)malloc(graph.idr[i] * sizeof(Line));
        ...
        graph.out[i] = (Line*)malloc(graph.odr[i] * sizeof(Line));
