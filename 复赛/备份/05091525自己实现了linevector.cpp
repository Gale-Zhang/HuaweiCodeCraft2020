    ...
    while(cur < bufLen) {
        ...
        if(graph.out[stIdx].len == graph.out[stIdx].maxLen) {
            if(graph.out[stIdx].maxLen == 0) {
                ...
                graph.out[stIdx].line = (Line*)realloc(graph.out[stIdx].line, graph.out[stIdx].maxLen);
            }
        }
        if(graph.in[endIdx].len == graph.in[endIdx].maxLen) {
            if(graph.in[endIdx].maxLen == 0) {
                ...
                graph.in[endIdx].line = (Line*)realloc(graph.in[endIdx].line, graph.in[endIdx].maxLen);
            }
        }
