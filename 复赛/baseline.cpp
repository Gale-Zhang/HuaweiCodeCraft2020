...
void loadData(string filename) {
    ...
    int cur = 0;
    unordered_map<int, int> idx{{},VNUM};
    ...
    while(cur < bufLen) {
        ...
        graph.out[stIdx][graph.odr[stIdx]][0] = end;
        graph.out[stIdx][graph.odr[stIdx]++][1] = cost;
        graph.in[endIdx][graph.idr[endIdx]][0] = st;
        graph.in[endIdx][graph.idr[endIdx]++][1] = cost;
    }
    ...
    for(int i = 0; i < graph.pnum; i++) {
        graph.list[i] = idx[graph.list[i]];
    }

    for(int i = 0; i < graph.pnum; i++) {
        sort(graph.out[i], graph.out[i] + graph.odr[i]);
        for(int j = 0; j < graph.odr[i]; j++) {
            graph.out[i][j] = idx[graph.out[i][j]];
        }
    }
    for(int i = 0; i < graph.pnum; i++) {
        sort(graph.in[i], graph.in[i] + graph.idr[i]);
        for(int j = 0; j < graph.idr[i]; j++) {
            graph.in[i][j] = idx[graph.in[i][j]];
        }
    }
...
void prune(int cur, int target, bool *visit, char *distance, int *cost, int dis, int *update, int &udlen) {
    for(int i = 0; i < graph.idr[cur]; ++i) {
        int t = graph.in[cur][i][0];

        if(t < target || visit[t]) continue;

        if(dis < distance[t]) {
            distance[t] = dis;
            cost[t] = graph.in[cur][i][1];
            update[udlen++] = t;
        }
...
void dfs(int cur, int target, bool *visit, char *distance, int *cost, int **path, int len, int idx) {
    for(int i = 0; i < graph.odr[cur]; ++i) {
        int t = graph.out[cur][i][0];

        if((len > 3 && (len + distance[t] > 7)) || visit[t] || t < target) continue;
        //180ms

        path[len][0] = t;
        path[len - 1][1] = graph.out[cur][i][1];
